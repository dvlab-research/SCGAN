...
class Pix2PixEMAModel(nn.Module):
    @staticmethod
            ...
            with torch.no_grad():
                if not self.opt.no_ema:
                    ...
                    if self.opt.use_vae and (not self.opt.random_z):
                        z, mu, logvar = self.encode_z(real_image)
                    fake_image, _, _ = self.netEMA(input_semantics, z)
                else:
    ...
    def save(self, epoch):
        ...
        if not self.opt.no_ema:
            util.save_network(self.netEMA, 'A', epoch, self.opt)
    ...
    def initialize_networks(self, opt):
        ...
        with torch.no_grad():
            netEMA = deepcopy(netG) if not opt.no_ema else None
        ...
        if not opt.isTrain or opt.continue_train:
            ...
            if not opt.no_ema:
                print('load EMA')
                netEMA = util.load_network(netEMA, 'A', opt.which_epoch, opt)
