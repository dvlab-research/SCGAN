...
class CocoDataset(Pix2pixDataset):
    ...
    def get_paths(self, opt):
        ...
        label_paths = make_dataset(label_dir, recursive=False, read_cache=False)

        image_dir = os.path.join(root, '%s_img' % phase)
        image_paths = make_dataset(image_dir, recursive=False, read_cache=False)

        if not opt.no_instance:
            instance_dir = os.path.join(root, '%s_inst' % phase)
            instance_paths = make_dataset(instance_dir, recursive=False, read_cache=False)
        else:
            instance_paths = []
